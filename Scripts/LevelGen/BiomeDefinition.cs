using System;
using System.Collections.Generic;
using System.Linq;
using Godot;
using SpacedOut.Sector;

namespace SpacedOut.LevelGen;

public class BiomeDefinition
{
    public string Id { get; init; } = "";
    public string DisplayName { get; init; } = "";
    public float LevelRadius { get; init; } = 400f;

    // Landmark tier
    public string[] LandmarkAssets { get; init; } = Array.Empty<string>();
    public int LandmarkCount { get; init; } = 1;

    // Mid-scale tier
    public string[] MidScaleAssets { get; init; } = Array.Empty<string>();
    public int MidScaleMin { get; init; } = 10;
    public int MidScaleMax { get; init; } = 20;

    // Small / scatter tier
    public string[] SmallAssets { get; init; } = Array.Empty<string>();
    public int SmallMin { get; init; } = 30;
    public int SmallMax { get; init; } = 60;

    // Scatter (loose objects outside clusters)
    public int ScatterMin { get; init; } = 10;
    public int ScatterMax { get; init; } = 30;

    // Markers (POI, loot, beacons)
    public string[] MarkerAssets { get; init; } = Array.Empty<string>();
    public int MarkerCount { get; init; } = 2;

    // Cluster settings
    public int ClusterCountMin { get; init; } = 3;
    public int ClusterCountMax { get; init; } = 5;
    public float ClusterRadius { get; init; } = 55f;
    public float ClusterSpacing { get; init; } = 80f;

    // Layout settings
    public float SpawnSafeRadius { get; init; } = 45f;
    public float LandmarkCenterOffset { get; init; } = 0.25f;
    public float CorridorWidth { get; init; } = 25f;

    // Resource zone templates (generated by SectorGenerator)
    public ResourceZoneTemplate[] ResourceZoneTemplates { get; init; } = Array.Empty<ResourceZoneTemplate>();

    // ─── Static biome registry ──────────────────────────────────────

    private static readonly Dictionary<string, BiomeDefinition> Biomes = new()
    {
        ["asteroid_field"] = new BiomeDefinition
        {
            Id = "asteroid_field",
            DisplayName = "Asteroidenfeld",
            LevelRadius = 1200f,

            LandmarkAssets = new[] { "asteroid_large" },
            LandmarkCount = 1,

            MidScaleAssets = new[] { "asteroid_medium" },
            MidScaleMin = 40,
            MidScaleMax = 65,

            SmallAssets = new[] { "asteroid_small" },
            SmallMin = 90,
            SmallMax = 160,

            ScatterMin = 40,
            ScatterMax = 80,

            // Asteroid-specific + cross-biome wildcard POIs (see PoiBlueprintCatalog for interaction logic).
            MarkerAssets = new[]
            {
                "poi_rich_vein", "poi_crystal_geode", "poi_fissure_cavity",
                "poi_drifting_pod", "poi_anomaly_source", "poi_wreck_fragment",
            },
            MarkerCount = 2,

            ClusterCountMin = 6,
            ClusterCountMax = 10,
            ClusterRadius = 100f,
            ClusterSpacing = 150f,

            SpawnSafeRadius = 80f,
            LandmarkCenterOffset = 0.35f,
            CorridorWidth = 50f,

            ResourceZoneTemplates = new[]
            {
                new ResourceZoneTemplate
                {
                    ResourceType = ResourceFieldType.Ore,
                    CountMin = 1, CountMax = 3,
                    RadiusMin = 100f, RadiusMax = 250f,
                    DensityMin = 0.4f, DensityMax = 0.9f,
                    MapColor = new Color(0.9f, 0.55f, 0.15f),
                    FillAssets = new[] { "asteroid_small", "asteroid_medium" },
                    BaseFillCount = 30,
                },
                new ResourceZoneTemplate
                {
                    ResourceType = ResourceFieldType.Crystal,
                    CountMin = 0, CountMax = 1,
                    RadiusMin = 80f, RadiusMax = 150f,
                    DensityMin = 0.3f, DensityMax = 0.7f,
                    MapColor = new Color(0.3f, 0.85f, 0.95f),
                    FillAssets = new[] { "asteroid_small" },
                    BaseFillCount = 20,
                },
            },
        },

        ["wreck_zone"] = new BiomeDefinition
        {
            Id = "wreck_zone",
            DisplayName = "Wrackzone",
            LevelRadius = 1000f,

            LandmarkAssets = new[] { "wreck_main" },
            LandmarkCount = 1,

            MidScaleAssets = new[] { "wreck_medium" },
            MidScaleMin = 35,
            MidScaleMax = 55,

            SmallAssets = new[] { "debris_cluster" },
            SmallMin = 80,
            SmallMax = 140,

            ScatterMin = 30,
            ScatterMax = 60,

            MarkerAssets = new[]
            {
                "beacon", "loot_marker",
                "poi_drifting_pod", "poi_anomaly_source", "poi_wreck_fragment",
            },
            MarkerCount = 2,

            ClusterCountMin = 8,
            ClusterCountMax = 12,
            ClusterRadius = 80f,
            ClusterSpacing = 130f,

            SpawnSafeRadius = 70f,
            LandmarkCenterOffset = 0.3f,
            CorridorWidth = 40f,

            ResourceZoneTemplates = new[]
            {
                new ResourceZoneTemplate
                {
                    ResourceType = ResourceFieldType.Scrap,
                    CountMin = 1, CountMax = 2,
                    RadiusMin = 80f, RadiusMax = 180f,
                    DensityMin = 0.5f, DensityMax = 0.85f,
                    MapColor = new Color(0.5f, 0.5f, 0.55f),
                    FillAssets = new[] { "debris_cluster", "wreck_medium" },
                    BaseFillCount = 25,
                },
                new ResourceZoneTemplate
                {
                    ResourceType = ResourceFieldType.Plasma,
                    CountMin = 0, CountMax = 1,
                    RadiusMin = 60f, RadiusMax = 120f,
                    DensityMin = 0.2f, DensityMax = 0.6f,
                    MapColor = new Color(0.7f, 0.3f, 0.9f),
                    FillAssets = new[] { "debris_cluster" },
                    BaseFillCount = 15,
                },
            },
        },

        ["station_periphery"] = new BiomeDefinition
        {
            Id = "station_periphery",
            DisplayName = "Stationsperipherie",
            LevelRadius = 950f,

            LandmarkAssets = new[] { "station_core" },
            LandmarkCount = 1,

            MidScaleAssets = new[] { "station_module" },
            MidScaleMin = 25,
            MidScaleMax = 45,

            SmallAssets = new[] { "cargo_cluster" },
            SmallMin = 65,
            SmallMax = 110,

            ScatterMin = 20,
            ScatterMax = 45,

            MarkerAssets = new[]
            {
                "utility_node",
                "poi_drifting_pod", "poi_anomaly_source", "poi_wreck_fragment",
            },
            MarkerCount = 2,

            ClusterCountMin = 5,
            ClusterCountMax = 9,
            ClusterRadius = 75f,
            ClusterSpacing = 120f,

            SpawnSafeRadius = 80f,
            LandmarkCenterOffset = 0.3f,
            CorridorWidth = 48f,

            ResourceZoneTemplates = new[]
            {
                new ResourceZoneTemplate
                {
                    ResourceType = ResourceFieldType.Gas,
                    CountMin = 0, CountMax = 1,
                    RadiusMin = 60f, RadiusMax = 130f,
                    DensityMin = 0.3f, DensityMax = 0.6f,
                    MapColor = new Color(0.2f, 0.8f, 0.4f),
                    FillAssets = new[] { "cargo_cluster" },
                    BaseFillCount = 15,
                },
            },
        },
    };

    public static BiomeDefinition Get(string id) => Biomes[id];

    public static bool TryGet(string id, out BiomeDefinition definition) =>
        Biomes.TryGetValue(id, out definition!);

    public static string[] GetAllIds() => Biomes.Keys.ToArray();
    public static IReadOnlyDictionary<string, BiomeDefinition> GetAll() => Biomes;
}
